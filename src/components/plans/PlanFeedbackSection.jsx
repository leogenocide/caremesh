...
export const PlanFeedbackSection = ({ plan }) => {
  const { currentUser, addPlanFeedback, markFeedbackStatus } = useCareMesh();
  ...
  const isProposer = plan.proposer?.id === currentUser.id || plan.participants?.some(p => p.user?.id === currentUser.id);
            ...
            >
              ...
              <div className="d-flex align-center justify-between flex-wrap gap-2 mb-2">
                <div className="d-flex align-center gap-2">
                  <img
                    src={item.author?.avatar || currentUser.avatar}
                    alt=""
                    style={{ width: '28px', height: '28px', borderRadius: '50%', objectFit: 'cover' }}
                  />
                  <div>
                    <div className="d-flex align-center gap-2">
                      <span className="font-bold text-xs text-primary">{item.author?.name}</span>
                      {item.author?.role && (
